final readStories = [
  {
    'id': '1',
    'imageUrl': "https://picsum.photos/id/11/200/300",
    'title': 'Title 1',
    'chapter': 'Chapter 1',
    "latitud": -8.079058,
    "longitud": -79.121091,
  },
  {
    'id': '2',
    'imageUrl': "https://picsum.photos/id/12/200/300",
    'title': 'Title 2',
    'chapter': 'Chapter 2',
    "latitud": 38.125815,
    "longitud": -122.250528,
  },
  {
    'id': '3',
    'imageUrl': "https://picsum.photos/id/13/200/300",
    'title': 'Title 3',
    'chapter': 'Chapter 3',
    "latitud": -8.119165,
    "longitud": -79.027283,
  },
  {
    'id': '4',
    'imageUrl': "https://picsum.photos/id/14/200/300",
    'title': 'Title 4',
    'chapter': 'Chapter 4',
    "latitud": -8.133169,
    "longitud": -78.995526,
  },
  {
    'id': '5',
    'imageUrl': "https://picsum.photos/id/15/200/300",
    'title': 'Title 5',
    'chapter': 'Chapter 5',
    "latitud": -8.195384,
    "longitud": -78.980658,
  },
  {
    'id': '6',
    'imageUrl': "https://picsum.photos/id/16/200/300",
    'title': 'Title 6',
    'chapter': 'Chapter 6',
    "latitud": -8.108148,
    "longitud": -79.042075,
  }
  // Más historias leídas
];

final unreadStories = [
  {
    'id': '7',
    'imageUrl': 'https://picsum.photos/id/17/200/300',
    'title': 'Title 7',
    'synopsis': 'Synopsis 7',
    'rating': 4.5,
    'reads': 10000,
    'distance': 'a 2 km',
    'isFavorite': false,
    "latitud": -8.079058,
    "longitud": -79.121091,
  },
  {
    'id': '8',
    'imageUrl': 'https://picsum.photos/id/18/200/300',
    'title': 'Title 8',
    'synopsis': 'Synopsis 8',
    'rating': 3.5,
    'reads': 20000,
    'distance': 'a 4 km',
    'isFavorite': false,
    "latitud": -8.014978,
    "longitud": -78.832017,
  },
  {
    'id': '9',
    'imageUrl': 'https://picsum.photos/id/19/200/300',
    'title': 'Title 9',
    'synopsis': 'Synopsis 9',
    'rating': 4.0,
    'reads': 30000,
    'distance': 'a 6 km',
    'isFavorite': false,
    "latitud": -8.120568,
    "longitud": -79.045331,
  },
  {
    'id': '10',
    'imageUrl': 'https://picsum.photos/id/20/200/300',
    'title': 'Title 10',
    'synopsis': 'Synopsis 10',
    'rating': 3.0,
    'reads': 40000,
    'distance': 'a 8 km',
    'isFavorite': false,
    "latitud": -8.132049,
    "longitud": -79.051103,
  },
  {
    'id': '11',
    'imageUrl': 'https://picsum.photos/id/21/200/300',
    'title': 'Title 11',
    'synopsis': 'Synopsis 11',
    'rating': 4.0,
    'reads': 50000,
    'distance': 'a 10 km',
    'isFavorite': false,
    "latitud": -8.079038,
    "longitud": -79.121011,
  },
  {
    'id': '12',
    'imageUrl': 'https://picsum.photos/id/22/200/300',
    'title': 'Title 12',
    'synopsis': 'Synopsis 12',
    'rating': 3.0,
    'reads': 60000,
    'distance': 'a 12 km',
    'isFavorite': false,
    "latitud": -8.120296,
    "longitud": -79.043255,
  },
  {
    'id': '13',
    'imageUrl': 'https://picsum.photos/id/23/200/300',
    'title': 'Title 13',
    'synopsis': 'Synopsis 13',
    'rating': 4.0,
    'reads': 70000,
    'distance': 'a 14 km',
    'isFavorite': false,
    "latitud": -8.079090,
    "longitud": -79.121028,
  },
  {
    'id': '14',
    'imageUrl': 'https://picsum.photos/id/24/200/300',
    'title': 'Title 14',
    'synopsis': 'Synopsis 14',
    'rating': 3.0,
    'reads': 80000,
    'distance': 'a 16 km',
    'isFavorite': false,
    "latitud": -8.079110,
    "longitud": -79.121038,
  },
  {
    'id': '15',
    'imageUrl': 'https://picsum.photos/id/25/200/300',
    'title': 'Title 15',
    'synopsis': 'Synopsis 15',
    'rating': 4.0,
    'reads': 90000,
    'distance': 'a 18 km',
    'isFavorite': false,
    "latitud": -8.102607,
    "longitud": -79.039027,
  },
  {
    'id': '16',
    'imageUrl': 'https://picsum.photos/id/26/200/300',
    'title': 'Title 16',
    'synopsis': 'Synopsis 16',
    'rating': 3.0,
    'reads': 100000,
    'distance': 'a 20 km',
    'isFavorite': false,
    "latitud": -8.120324,
    "longitud": -79.045738,
  }
  // Más historias no leídas
];

final distanceData = {
  'destination_addresses': [
    'Av la Ribera 361, Huanchaco, Peru',
    '10A 280, 13100, Peru',
    'VXH3+QWW, Trujillo 13011, Peru',
    'VW9X+4HV, Víctor Larco Herrera 13009, Peru',
    'Av la Ribera 361, Huanchaco, Peru',
    'VXH4+XM6, C. R.P. Jaime de Gondra, Víctor Larco Herrera 13011, Peru',
    'Av la Ribera 361, Huanchaco, Peru',
    'Av la Ribera 361, Huanchaco, Peru',
    'Av. Mansiche 1070, Trujillo 13011, Peru',
    'La Rivera 820, Huanchaco 13001, Peru'
  ],
  'origin_addresses': [
    'Av. Antenor Orrego 1265, Víctor Larco Herrera 13011, Peru'
  ],
  'rows': [
    {
      'elements': [
        {
          'distance': {'text': '15.9 km', 'value': 15882},
          'duration': {'text': '25 mins', 'value': 1516},
          'status': 'OK'
        },
        {
          'distance': {'text': '31.8 km', 'value': 31829},
          'duration': {'text': '47 mins', 'value': 2801},
          'status': 'OK'
        },
        {
          'distance': {'text': '73 m', 'value': 73},
          'duration': {'text': '1 min', 'value': 29},
          'status': 'OK'
        },
        {
          'distance': {'text': '2.0 km', 'value': 2035},
          'duration': {'text': '6 mins', 'value': 383},
          'status': 'OK'
        },
        {
          'distance': {'text': '15.9 km', 'value': 15874},
          'duration': {'text': '25 mins', 'value': 1514},
          'status': 'OK'
        },
        {
          'distance': {'text': '31.8 km', 'value': 31819},
          'duration': {'text': '47 mins', 'value': 2800},
          'status': 'OK'
        }
      ],
      'status': 'OK'
    }
  ]
};
